 ...
 * @returns {object}
 */
async function renameAccount(oldNickname, newNickname){
    let oldFolder = ""
    let newFolder = ""
    let newFilePath = ""

    try {
        oldFolder = path.join(constants.multiUserPath, oldNickname)
        newFolder = path.join(constants.multiUserPath, newNickname)
        newFilePath = path.join(newFolder, 'user.bin')
    } catch (e) {
        return error.PATH_SET_ERROR
    }

    if (await fileFuncs.checkExists(oldFolder)) {
        if (!await fileFuncs.moveFile(oldFolder, newFolder)) {
            return error.MOVE_FILE_ERROR
        }
    }

    global_state.data.rename(oldNickname, newNickname, newFilePath)

    if (global_state.config.currentUser === oldNickname) {
        global_state.config.currentUser = newNickname
    }
