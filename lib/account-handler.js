...
const {checkNickname,removeAccount,switchAccountData,renameAccount,getAccountList } = require('./account-data')
 ...
 * @returns {object}
 */
async function renameAccountHandler(oldNickname, newNickname){
    if(!checkNickname(oldNickname)){
        return error.NICKNAME_NOT_EXISTS
    }
    if(checkNickname(newNickname)){
        return error.NICKNAME_EXITS
    }
    const error1 = await renameAccount(oldNickname, newNickname)
    if(error1){
        return error1
    }
